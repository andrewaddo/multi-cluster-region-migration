...
class MetricsCollector:
    ...
    def add_result(self, timestamp, latency, status_code, cluster, region, error=None):
        with self.lock:
            self.results.append({
                'timestamp': timestamp,
                'latency': latency,
...
def worker(url, collector, stop_event, rate_limit_sleep):
    while not stop_event.is_set():
        ...
        try:
            # Disable keep-alive to force fresh connections for better LB visibility
            ...
            collector.add_result(timestamp, latency, status_code, cluster, region)
...
def main():
    parser = argparse.ArgumentParser(description="Performance Load Tester for Region Migration")
    ...
    # Calculate sleep for RPS (approximate)
